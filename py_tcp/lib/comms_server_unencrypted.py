...
class Server(socketserver.TCPServer):
    ...
    class Handler(socketserver.BaseRequestHandler):
        def handle(self):
            ...
            k, args, kwargs = bytes_to_object(b_in[IDENTIFIER_LEN:])
            ...
            if k in self.server.client_functions:
                ...
                if func:=getattr(self.server, k, None):
                    ...
                    b_out = object_to_bytes(result)
                    self.request.sendall(b_out)

    def __init__(self, address, path, buffer=4096):
...
##    key = load_key(f"{local_dir}/{key_name}")
    with server_class(address, local_dir, buffer) as s:
